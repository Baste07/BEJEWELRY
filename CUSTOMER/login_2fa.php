...
$error = $_SESSION['login_2fa_error'] ?? '';
unset($_SESSION['login_2fa_error']);
?>
<!DOCTYPE html>
<html lang="en">
<head>
<meta charset="UTF-8"><meta name="viewport" content="width=device-width,initial-scale=1.0">
<title>Authenticator — Bejewelry</title>
...
<div class="login-wrap">
  <div class="login-brand">
    <div>
      ...
      <div class="brand-sub" style="font-size:.6rem;font-weight:600;letter-spacing:.22em;text-transform:uppercase;margin-bottom:48px">Two-step sign-in</div>
      <p class="brand-body" style="font-size:.9rem;line-height:1.85">Enter the 6-digit code from your Google Authenticator app to finish signing in.</p>
    </div>
  </div>
  <div class="login-form-wrap">
    <div class="login-form-inner">
      <h2 style="font-size:1.6rem;margin-bottom:4px">Authenticator code</h2>
      <p style="margin-bottom:24px;font-size:.85rem;color:var(--muted)">Open Google Authenticator and enter the code for Bejewelry.</p>
      <?php if ($error !== ''): ?>
      <p class="muted" style="margin-bottom:16px;color:var(--danger)"><?= htmlspecialchars($error, ENT_QUOTES, 'UTF-8') ?></p>
      <?php endif; ?>
      <form action="verify_2fa_login.php" method="post" autocomplete="off" id="otpForm">
          <?php echo csrf_token_field(); ?>
        <div class="fg">
          <label class="flabel">6-digit code</label>
          <input type="hidden" name="totp" id="totpHidden" required>
          <div class="otp-group" id="otpGroup" role="group" aria-label="6-digit authenticator code">
        ...
        <button type="submit" class="btn btn-primary btn-full btn-lg" style="justify-content:center;margin-top:8px">Verify & continue</button>
      </form>
      <p class="tc mt5 t-sm muted"><a href="login.php" class="text-rose">← Back to sign in</a></p>
    </div>
  </div>
</div>
<script>
(function() {
  ...
  var boxes = Array.from(document.querySelectorAll('#otpGroup .otp-box'));
