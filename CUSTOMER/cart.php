...
.btn-primary:hover{transform:translateY(-1px);box-shadow:0 6px 20px rgba(176,48,80,.4)}
...
.btn-full{width:100%}
/* ── CART ACTIONS ─── */
.cart-remove{background:none;border:none;cursor:pointer;font-size:.72rem;color:var(--danger);margin-top:8px;padding:0;transition:all var(--tr)}
.cart-remove:hover{color:var(--danger-deep);font-weight:600;opacity:.8}
.cart-clear-btn{transition:all var(--tr)}
.cart-clear-btn:hover{opacity:.8;transform:scale(1.05)}
...
.qty-btn{width:33px;height:33px;background:var(--blush);border:none;cursor:pointer;font-size:1rem;color:var(--rose-deep);display:flex;align-items:center;justify-content:center;transition:background .15s;flex-shrink:0}
...
.citem{display:grid;grid-template-columns:56px 1fr auto;gap:12px;align-items:start;padding:12px 0;border-bottom:1px solid var(--border)}
.citem:last-child{border-bottom:none}
.ci-img{width:56px;height:56px;border-radius:var(--r-md);background:var(--blush-mid);display:flex;align-items:center;justify-content:center;font-size:1.45rem;flex-shrink:0}
.ci-name{font-family:var(--fd);font-size:.87rem;font-weight:500;color:var(--dark);margin-bottom:2px}
...
$checkoutDefaults = [
  ...
  'paymongo_enabled' => function_exists('paymongo_is_configured') ? paymongo_is_configured() : false,
];
error_log('[cart.php] Initial defaults: ' . json_encode($checkoutDefaults));
error_log('[cart.php] File exists check for _settings_db.php: ' . (__DIR__ . '/admin/settings/_settings_db.php'));

try {
  $settingsDbPath = __DIR__ . '/admin/settings/_settings_db.php';
  if (file_exists($settingsDbPath)) {
    error_log('[cart.php] Loading settings file...');
    require_once $settingsDbPath;
    $sPdo = settingsPdo();
    error_log('[cart.php] Got PDO connection');
    ...
    error_log('[cart.php] DB shipping result: ' . json_encode($dbShipping));
    ...
    error_log('[cart.php] Loaded shipping from DB: ' . json_encode($checkoutDefaults));
  } else {
    error_log('[cart.php] Settings DB file does not exist at: ' . $settingsDbPath);
  }
} catch (Throwable $e) {
  error_log('[cart.php] Exception: ' . $e->getMessage() . ' at ' . $e->getFile() . ':' . $e->getLine());
}
?>
<script>
...
console.log('[cart] window.__CHECKOUT__ =', window.__CHECKOUT__);
</script>
<div id="toasts"></div>
...
<div class="js-header" data-active="cart"></div>
...
<div class="main-content"><div class="container">
  ...
  <div class="page-hdr">
    <div class="bc"><a href="index.php">Home</a><span class="sep">›</span><span class="cur">Shopping Cart</span></div>
    <h1>Your Cart</h1>
  </div>

  <!-- EMPTY STATE -->
  <div id="cartEmpty" class="hidden" style="text-align:center;padding:80px 0">
    <div style="font-size:4rem;margin-bottom:16px">🛍️</div>
    <h2 style="margin-bottom:8px">Your cart is empty</h2>
    <p style="margin-bottom:24px">Discover our beautiful jewelry collection</p>
    <a href="product-list.php" class="btn btn-primary btn-lg">Shop Now</a>
  </div>

  <!-- CART CONTENT -->
  <div id="cartContent" class="checkout-layout">
    <div>
      <div class="card">
        <div class="card-hd"><h3>Order Items</h3><span class="t-sm muted" id="itemCount"></span></div>
        <div style="padding:0 var(--s6)" id="cartItemsList"></div>
      </div>
      <div class="flex jb items-center mt4">
        <a href="product-list.php" class="btn btn-ghost btn-sm">Continue Shopping</a>
        <button class="btn btn-ghost btn-sm cart-clear-btn" style="color:var(--danger)" onclick="if(confirm('Clear entire cart?')){clearCart();renderPage()}">🗑 Clear Cart</button>
      </div>
    </div>
    ...
    <div>
      <div class="sum-box">
        ...
        <div class="sum-total" id="summaryTotal"></div>
        <div style="margin-top:var(--s5)">
          <a href="checkout.php" class="btn btn-primary btn-full" style="justify-content:center">Proceed to Checkout</a>
        </div>
      </div>
      ...
      <div class="card mt4">
        <div class="card-body" style="display:grid;grid-template-columns:1fr 1fr;gap:12px">
          <div class="flex items-center g-2"><span>🔒</span><span class="t-xs muted">Secure Payment</span></div>
          <div class="flex items-center g-2"><span>🚚</span><span class="t-xs muted">Free Shipping ₱2k+</span></div>
          <div class="flex items-center g-2"><span>↩️</span><span class="t-xs muted">7-Day Returns</span></div>
          <div class="flex items-center g-2"><span>💎</span><span class="t-xs muted">Certified Authentic</span></div>
        </div>
      </div>
    </div>
  </div>
...
function renderPage() {
  ...
  document.getElementById('cartItemsList').innerHTML = cart.map(item => {
    const img = item.image_url ? '<img src="' + escapeAttr(item.image_url) + '" alt="' + escapeAttr(item.name) + '" style="width:100%;height:100%;object-fit:cover;border-radius:inherit">' : '<span style="font-size:1.5rem;font-weight:700;color:var(--rose-muted)">' + escapeHtml((item.cat||'?')[0]) + '</span>';
    return `
    <div style="display:grid;grid-template-columns:72px 1fr auto;gap:16px;align-items:start;padding:20px 0;border-bottom:1px solid var(--border)">
      <div class="iph sm" style="width:72px;height:72px;border-radius:var(--r-md);overflow:hidden">${img}</div>
      <div>
        <div style="font-family:var(--fd);font-size:.97rem;font-weight:500;margin-bottom:3px">${escapeHtml(item.name)}</div>
        <div class="t-xs muted mb3">${escapeHtml(item.cat || '')} · ${escapeHtml(item.size)}</div>
        <div class="qty">
          <button class="qty-btn" onclick="updateQtyById(${item.id}, ${item.qty}-1)">−</button>
          <span class="qty-val">${item.qty}</span>
          <button class="qty-btn" onclick="updateQtyById(${item.id}, ${item.qty}+1)">+</button>
        </div>
      </div>
      <div style="text-align:right">
        <div style="font-family:var(--fd);font-size:1rem;font-weight:600;margin-bottom:6px">₱${(item.price * item.qty).toLocaleString()}</div>
        <div class="t-xs muted">₱${Number(item.price).toLocaleString()} each</div>
        <a href="cart_remove.php?id=${item.id}&redirect=cart.php" class="cart-remove">Remove</a>
      </div>
    </div>`;
  }).join('');

  const subtotal = cart.reduce((a, i) => a + i.price * i.qty, 0);
  const cfg = window.__CHECKOUT__ || { shipping_fee: <?= defined('SHIPPING_FEE') ? (int)SHIPPING_FEE : 150 ?>, free_ship_threshold: <?= defined('FREE_SHIP_THRESHOLD') ? (int)FREE_SHIP_THRESHOLD : 2000 ?> };
  const shipping = subtotal >= (Number(cfg.free_ship_threshold) || 0) ? 0 : (Number(cfg.shipping_fee) || 0);
  const total    = subtotal + shipping;

  document.getElementById('summaryRows').innerHTML = `
    <div class="sum-row"><span class="sl">Subtotal (${cart.length} item${cart.length!==1?'s':''})</span><span class="sv">₱${subtotal.toLocaleString()}</span></div>
    <div class="sum-row"><span class="sl">Shipping</span><span class="sv" style="color:${shipping===0?'var(--success)':'var(--dark)'}">${shipping===0?'FREE':'₱'+shipping}</span></div>`;
  document.getElementById('summaryTotal').innerHTML = `<span class="stl">Total</span><span class="stv">₱${total.toLocaleString()}</span>`;
}

function updateQtyById(cartItemId, newQty) {
  newQty = Math.max(1, parseInt(newQty, 10) || 1);
  window.location = 'cart_update.php?id=' + cartItemId + '&qty=' + newQty + '&redirect=cart.php';
}
