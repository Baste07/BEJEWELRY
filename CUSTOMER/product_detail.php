...
$related = [];
if ($id) {
  $stmt = db()->prepare('SELECT p.*, c.name AS cat, c.slug AS cat_slug FROM products p LEFT JOIN categories c ON c.id = p.category_id WHERE p.id = ? AND p.is_active = 1');
  $stmt->execute([$id]);
  $product = $stmt->fetch();
  if ($product) {
    $product = format_product_for_customer($product);
    $product['size'] = $product['size_default'] ?? 'One Size';
    $product['orig'] = $product['orig_price'] ?? null;
    $relStmt = db()->prepare('SELECT p.*, c.name AS cat FROM products p LEFT JOIN categories c ON c.id = p.category_id WHERE p.category_id = (SELECT category_id FROM products WHERE id = ?) AND p.id != ? AND p.is_active = 1 ORDER BY p.reviews DESC LIMIT 5');
    $relStmt->execute([$id, $id]);
    $related = array_map('format_product_for_customer', $relStmt->fetchAll());
  }
}
$user = current_user();
...
<?= csrf_meta_tag() ?>
<title><?= $product ? htmlspecialchars($product['name']) . ' — Bejewelry' : 'Product — Bejewelry' ?></title>
<style>
/* ═══════════════════════════════════════════════════════════
   BEJEWELRY — Fonts
...
<style>
.size-btn{width:42px;height:42px;border-radius:var(--r-sm);border:1.5px solid var(--border-mid);background:var(--white);font-size:.84rem;font-weight:600;color:var(--dark);cursor:pointer;transition:all var(--tr)}
.size-btn:hover,.size-btn.active{background:var(--rose);color:var(--white);border-color:var(--rose)}
</style>
<script>
...
<div class="js-header" data-active="shop"></div>

<div class="site-wrapper">
<aside class="sidebar" data-active="shop"></aside>
<div class="site-content">
<div class="main-content"><div class="container">

  <div class="bc mt4">
    <a href="index.php">Home</a><span class="sep">›</span>
    <a href="product-list.php">All Jewellery</a><span class="sep">›</span>
    <span class="cur" id="bcName">Loading…</span>
  </div>

  <!-- PRODUCT DETAIL -->
  <div class="g2 mt5" style="align-items:flex-start">
    <!-- IMAGE -->
    <div>
      <div class="iph main" id="mainImg" style="cursor:zoom-in;border-radius:var(--r-xl)"></div>
      <div style="display:grid;grid-template-columns:repeat(4,1fr);gap:10px;margin-top:12px" id="thumbRow"></div>
    </div>

    <!-- INFO -->
    <div id="productInfo">
      <span class="eyebrow" id="pdCat"></span>
      <h1 style="font-size:2rem;margin-bottom:8px" id="pdName"></h1>
      <div class="flex items-center g-4 mb4">
        <span style="font-size:.9rem;color:var(--gold)" id="pdStars"></span>
        <span class="t-xs muted" id="pdReviews"></span>
        <span class="badge b-delivered" id="pdStockBadge">In Stock</span>
      </div>
      <div style="font-family:var(--fd);font-size:2.2rem;font-weight:700;color:var(--dark);margin-bottom:6px" id="pdPrice"></div>
      <p style="margin-bottom:24px;line-height:1.8" id="pdDesc"></p>

      <!-- SIZE PICKER (shown for Rings only) -->
      <div style="margin-bottom:20px" id="sizeSection">
        <div class="flabel" style="margin-bottom:8px">Ring Size
          <a href="#" style="font-size:.7rem;color:var(--rose);margin-left:8px" onclick="event.preventDefault();toast('Tip: measure your finger circumference in mm')">Size Guide</a>
        </div>
        <div class="flex g-2 flex-wrap" id="sizePicker">
          <button class="size-btn" onclick="pickSize(this)">5</button>
          <button class="size-btn active" onclick="pickSize(this)">6</button>
          <button class="size-btn" onclick="pickSize(this)">7</button>
          <button class="size-btn" onclick="pickSize(this)">8</button>
          <button class="size-btn" onclick="pickSize(this)">9</button>
        </div>
      </div>

      <!-- QTY -->
      <div class="flex items-center g-4 mb4">
        <div class="flabel" style="margin-bottom:0">Qty</div>
        <div class="qty">
          <button class="qty-btn" id="qtyDecBtn" onclick="changeQty(-1)">−</button>
          <input class="qty-val" type="number" id="detailQty" value="1" min="1" max="10" style="pointer-events:none">
          <button class="qty-btn" id="qtyIncBtn" onclick="changeQty(1)">+</button>
        </div>
      </div>

      <div style="display:flex;flex-direction:column;gap:10px;margin-bottom:24px">
        <button class="btn btn-primary btn-lg" id="addToCartBtn" onclick="handleAddToCart()">🛍️ Add to Cart</button>
        <div class="flex g-3">
          <button class="btn btn-secondary" style="flex:1" id="wishlistBtn" onclick="handleWishlist()">♡ Add to Wishlist</button>
          <button class="btn btn-dark" id="buyNowBtn" style="flex:1" onclick="handleBuyNow()">Buy Now</button>
        </div>
      </div>

      <!-- SPECS -->
      <div style="display:grid;grid-template-columns:1fr 1fr;gap:10px;background:var(--blush);border-radius:var(--r-md);padding:16px" id="pdSpecs"></div>
    </div>
  </div>

  <!-- RELATED PRODUCTS -->
  <div style="margin-top:52px">
    <h2 style="font-size:1.5rem;margin-bottom:24px">You May Also Like</h2>
    <div class="g4" id="relatedGrid"></div>
  </div>
...
<script>
  window.__PRODUCT__ = <?= json_encode($product, JSON_HEX_TAG | JSON_HEX_AMP | JSON_HEX_APOS | JSON_HEX_QUOT) ?>;
  window.__RELATED__ = <?= json_encode($related, JSON_HEX_TAG | JSON_HEX_AMP | JSON_HEX_APOS | JSON_HEX_QUOT) ?>;
...
document.addEventListener('DOMContentLoaded', () => {
  const p = window.__PRODUCT__ || null;
  currentProduct = p;
  if (!p) {
    document.getElementById('bcName').textContent = 'Product not found';
    document.getElementById('productInfo').innerHTML = '<p class="muted">Product not found.</p><a href="product-list.php" class="btn btn-primary">Back to Shop</a>';
    return;
  }
  document.getElementById('bcName').textContent  = p.name;
  document.getElementById('pdCat').textContent   = p.cat || '';
  document.getElementById('pdName').textContent   = p.name;
  document.getElementById('pdStars').textContent  = '★'.repeat(p.stars || 5) + '☆'.repeat(5 - (p.stars || 5));
  document.getElementById('pdReviews').textContent = '(' + (p.reviews || 0) + ' reviews)';
  document.getElementById('pdPrice').innerHTML = '₱' + Number(p.price).toLocaleString() + (p.orig_price ? ' <del style="font-size:1rem;color:var(--muted-light)">₱' + Number(p.orig_price).toLocaleString() + '</del>' : '');
  document.getElementById('pdDesc').textContent = (p.description || ('A beautiful piece crafted from ' + (p.material || 'fine materials') + '. The ' + p.name + ' is perfect for everyday wear or special occasions.'));
  const mainImg = document.getElementById('mainImg');
  if (p.image_url) {
    mainImg.innerHTML = '<img src="' + escapeAttr(p.image_url) + '" alt="' + escapeHtml(p.name) + '" style="width:100%;height:100%;object-fit:cover;border-radius:inherit">';
  } else {
    mainImg.textContent = (p.cat || p.name || '?')[0];
    mainImg.style.fontSize = '8rem';
    mainImg.style.fontWeight = '700';
    mainImg.style.color = 'var(--rose-muted)';
  }
  const thumbRow = document.getElementById('thumbRow');
  if (p.image_url) {
    thumbRow.innerHTML = '<div style="height:80px;border-radius:var(--r-md);cursor:pointer;border:2px solid var(--rose);overflow:hidden"><img src="' + escapeAttr(p.image_url) + '" alt="" style="width:100%;height:100%;object-fit:cover"></div>';
  } else {
    thumbRow.innerHTML = '<div style="height:80px;border-radius:var(--r-md);border:2px solid var(--rose);background:var(--blush-mid);display:flex;align-items:center;justify-content:center;font-size:1.6rem">' + (p.cat || p.name || '?')[0] + '</div>';
  }
  document.getElementById('pdSpecs').innerHTML = `
    <div><div class="t-xs muted" style="margin-bottom:3px">Material</div><div style="font-size:.84rem;font-weight:600">${escapeHtml(p.material || '—')}</div></div>
    <div><div class="t-xs muted" style="margin-bottom:3px">Category</div><div style="font-size:.84rem;font-weight:600">${escapeHtml(p.cat || '—')}</div></div>
    <div><div class="t-xs muted" style="margin-bottom:3px">Size</div><div style="font-size:.84rem;font-weight:600">${escapeHtml(p.size || p.size_default || 'One Size')}</div></div>
    <div><div class="t-xs muted" style="margin-bottom:3px">Delivery</div><div style="font-size:.84rem;font-weight:600">3–5 business days</div></div>`;
  if (p.cat !== 'Rings') document.getElementById('sizeSection').style.display = 'none';
  const wBtn = document.getElementById('wishlistBtn');
  if (isWished(p.id)) { wBtn.textContent = '♥ In Wishlist'; wBtn.style.background = 'var(--blush-mid)'; }
  ...
  if (stock <= 0) {
    if (stockBadge) {
      stockBadge.className = 'badge b-cancelled';
    ...
    qtyInp.max = String(Math.min(10, stock));
  }
  const related = (window.__RELATED__ || []).filter(x => x.id !== p.id).slice(0, 4);
  document.getElementById('relatedGrid').innerHTML = related.map(x => productCard(x, 'product_detail.php')).join('');
});

function setThumb(el, emoji) {
  document.getElementById('mainImg').textContent = emoji;
  document.querySelectorAll('#thumbRow > div').forEach(t => t.style.border = '1.5px solid var(--border)');
  el.style.border = '2px solid var(--rose)';
}
...
function changeQty(delta) {
  ...
  const maxQty = Math.max(1, Math.min(10, Number(currentProduct.stock || 1)));
  inp.value = Math.max(1, Math.min(maxQty, parseInt(inp.value) + delta));
}
...
async function handleAddToCart() {
  ...
  if (qty > Number(currentProduct.stock || 0)) {
    toast('Requested quantity exceeds available stock.', 'error');
    return;
  }
  const ok = await addToCart(currentProduct.id, qty, currentProduct.cat === 'Rings' ? `Size ${selectedSize}` : currentProduct.size);
  if (ok) openCart();
}
...
function handleWishlist() {
  ...
  btn.style.background = wished ? 'var(--blush-mid)' : '';
}
