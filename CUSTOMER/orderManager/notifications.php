...
if (!empty($prefs['new_order'])) {
	...
	foreach ($recent as $o) {
		addAlert($alerts, 'order', 'Order placed', 'Order #' . $o['id'] . ' · + ₱' . number_format((float)$o['total'], 0), 'orders.php?search=' . urlencode((string)$o['id']), $o['created_at'] ?? null);
	}
}
...
if (!empty($prefs['low_stock'])) {
	...
	if (count($lowStock) > 0) {
		addAlert($alerts, 'inventory', 'Low stock', count($lowStock) . ' product(s) are low on stock.', 'dashboard.php');
		foreach ($lowStock as $p) {
			addAlert($alerts, 'inventory', 'Low stock item', $p['name'] . ' — ' . (int)$p['stock'] . ' left', 'dashboard.php');
		}
	}
}
...
if (!empty($prefs['customer_reg'])) {
	...
	if ($newCust > 0) {
		addAlert($alerts, 'customer', 'New customers', $newCust . ' new registration(s) today.', 'dashboard.php');
	}
}
...
<head>
	...
	<title>Bejewelry Order Manager — Notifications</title>
