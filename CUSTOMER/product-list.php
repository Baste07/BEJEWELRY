...
$sql = "SELECT p.*, c.name AS cat, c.slug AS cat_slug FROM products p LEFT JOIN categories c ON c.id = p.category_id WHERE " . implode(' AND ', $where) . " ORDER BY " . $orderBy;
$stmt = db()->prepare($sql);
$stmt->execute($params);
$products = array_map('format_product_for_customer', $stmt->fetchAll());
$user = current_user();
...
<?= csrf_meta_tag() ?>
<title>Shop All Jewelry — Bejewelry</title>
<style>
/* ═══════════════════════════════════════════════════════════
   BEJEWELRY — Fonts
...
.pcard-price-wrap{display:inline-flex;align-items:baseline;gap:6px;flex-wrap:wrap}
...
/* ── CATEGORY CARD ─── */
.catcard{border-radius:var(--r-lg);overflow:visible;border:1px solid var(--border);cursor:pointer;transition:transform .22s cubic-bezier(.34,1.56,.64,1),box-shadow .22s ease,border-color .22s ease;display:block;background:var(--white);box-shadow:var(--sh-xs);position:relative;}
...
<script>
// Set active nav/sidebar from URL before page renders
(function(){
  const cat   = new URLSearchParams(location.search).get('cat');
  const badge = new URLSearchParams(location.search).get('badge');
  const map = { 'Rings':'rings','Necklaces':'necklaces','Bracelets':'bracelets','Earrings':'earrings' };
  const activeKey = cat ? (map[cat] || map[decodeURIComponent(cat.replace(/\+/g,' '))] || 'shop') : (badge === 'new' ? 'new' : badge === 'sale' ? 'sale' : 'shop');
  document.addEventListener('DOMContentLoaded', () => {
    document.querySelectorAll('.sidebar[data-active]').forEach(el => el.setAttribute('data-active', activeKey));
  });
})();
</script>
<script>
...
<body>
<script>
window.__PRODUCT_LIST_DATA__ = <?= json_encode(['products' => $products], JSON_HEX_TAG | JSON_HEX_AMP | JSON_HEX_APOS | JSON_HEX_QUOT) ?>;
...
<div class="js-header" id="mainHeader" data-active="shop"></div>

<div class="site-wrapper">
<aside class="sidebar" id="mainSidebar" data-active="shop"></aside>
<div class="site-content">
<div class="main-content"><div class="container">

  <div class="page-hdr">
    <div class="bc"><a href="index.php">Home</a><span class="sep">›</span><span class="cur" id="pageTitle">All Jewelry</span></div>
    <span class="eyebrow">Our Collection</span>
    <h1 id="pageHeading">Shop All Jewelry</h1>
  </div>

  <!-- CATEGORY TABS — built dynamically from PRODUCTS -->
  ...
  <div class="sidebar-layout">
    ...
    <div class="filter-col">
      <div class="fp">
        <div class="fp-hd">Sort By</div>
        <div class="fp-body">
          <select class="fselect" id="sortSel" onchange="renderGrid()">
      ...
      <div class="fp">
        <div class="fp-hd">Material</div>
        <div class="fp-body" id="materialFilter"></div>
      </div>
      <div class="fp">
        <div class="fp-hd">Price Range</div>
        <div class="fp-body">
          <label class="fp-opt"><input type="checkbox" value="under500"  class="price-filter" onchange="renderGrid()"> Under ₱500</label>
          <label class="fp-opt"><input type="checkbox" value="500to1000" class="price-filter" onchange="renderGrid()"> ₱500 – ₱1,000</label>
          <label class="fp-opt"><input type="checkbox" value="over1000"  class="price-filter" onchange="renderGrid()"> ₱1,000+</label>
        </div>
      </div>
      <button class="btn btn-ghost btn-full btn-sm mt4" onclick="resetFilters()">Reset Filters</button>
    </div>

    <!-- GRID -->
    <div class="content-col">
      <div class="toolbar" id="toolbarRow" style="display:none">
        <span class="toolbar-info" id="resultCount"></span>
      </div>
      <div class="g3" id="productGrid" style="display:none"></div>
      <div id="placeholderSections"></div>
    </div>
  </div>
...
function showPlaceholders() {
  const wrap = document.getElementById('placeholderSections');
  ...
  wrap.innerHTML = `
    <div style="padding:48px 0;text-align:center;color:var(--muted-light)">
      <div style="font-size:2.5rem;margin-bottom:12px">💎</div>
      <p class="t-sm">Products are loading. If this takes too long, please refresh the page.</p>
    </div>`;
...
function renderGrid() {
  ...
  const materials = [...document.querySelectorAll('.mat-filter:checked')].map(c => c.value);
  const prices    = [...document.querySelectorAll('.price-filter:checked')].map(c => c.value);

  let list = currentCat === 'all' ? [...allProducts] : allProducts.filter(p => p.cat === currentCat);

  if (materials.length) list = list.filter(p => materials.includes(p.material));
  if (prices.length) {
    list = list.filter(p => {
      if (prices.includes('under500')  && p.price < 500)                return true;
      if (prices.includes('500to1000') && p.price >= 500 && p.price <= 1000) return true;
      if (prices.includes('over1000')  && p.price > 1000)               return true;
      return false;
    });
  }
  ...
  if (!list.length) {
    const placeholderWrap = document.getElementById('placeholderSections');
    // If API returned nothing at all → show placeholders
    if (!allProducts.length) {
      showPlaceholders();
    } else {
      // Filters returned zero — show a clear message with reset button
      if (placeholderWrap) placeholderWrap.style.display = 'none';
      grid.style.display = '';
      grid.innerHTML = `<div style="grid-column:1/-1;text-align:center;padding:60px 0;color:var(--muted-light)">
        <div style="font-size:2.5rem;margin-bottom:12px">🔍</div>
        <p style="margin-bottom:16px">No products match your filters.</p>
  ...
  const placeholderWrap2 = document.getElementById('placeholderSections');
  if (placeholderWrap2) placeholderWrap2.style.display = 'none';
  const tb2 = document.getElementById('toolbarRow');
...
function filterCat(el, cat) {
  if (!cat && el) cat = el.dataset ? el.dataset.cat : '';
  ...
  if (heading) heading.textContent = cat === 'all' ? 'All Jewelry' : cat;
  if (!allProducts.length) showPlaceholders(currentCat, null);
  else renderGrid();
}

function resetFilters() {
  document.querySelectorAll('.mat-filter,.price-filter').forEach(i => i.checked = false);
  document.getElementById('sortSel').value = 'featured';
  currentCat = 'all';
  document.querySelector('.tab[data-cat="all"]')?.click();
  renderGrid();
}

document.addEventListener('DOMContentLoaded', async () => {
  // Show placeholders immediately before API responds
  showPlaceholders();
  ...
  // Update page title
  if (catParam) {
    document.getElementById('pageTitle').textContent   = catParam;
    document.getElementById('pageHeading').textContent = catParam;
  } else if (srchParam) {
    document.getElementById('pageTitle').textContent   = `"${srchParam}"`;
    document.getElementById('pageHeading').textContent = `Results for "${srchParam}"`;
  } else if (badgeParam === 'new') {
    document.getElementById('pageTitle').textContent   = 'New Arrivals';
    document.getElementById('pageHeading').textContent = 'New Arrivals';
  } else if (badgeParam === 'sale') {
    document.getElementById('pageTitle').textContent   = 'Sale';
    document.getElementById('pageHeading').textContent = 'Sale';
  } else if (badgeParam === 'best') {
    document.getElementById('pageTitle').textContent   = 'Best Sellers';
    document.getElementById('pageHeading').textContent = 'Best Sellers';
  }

  const data = window.__PRODUCT_LIST_DATA__ || {};
  allProducts = data.products || [];
  // Products are already filtered server-side using URL params.
  // Keep only client-side search fallback for safety.
  if (srchParam)  allProducts = allProducts.filter(p => p.name.toLowerCase().includes(srchParam.toLowerCase()));

  // Build category tabs from loaded products
  if (badgeParam) {
    // Badge pages (New Arrivals, Sale, etc.) — just show one tab, no category switching
    const badgeLabels = { new: 'New Arrivals', sale: 'Sale', best: 'Best Sellers' };
    const label = badgeLabels[badgeParam] || badgeParam;
    document.getElementById('catTabs').innerHTML =
      `<div class="tab active" data-cat="all">${label} <span class="tab-n">${allProducts.length}</span></div>`;
  } else {
    const cats = ['all', ...new Set(allProducts.map(p => p.cat))];
    document.getElementById('catTabs').innerHTML = cats.map(cat => {
      const count = cat === 'all' ? allProducts.length : allProducts.filter(p => p.cat === cat).length;
      const label = cat === 'all' ? 'All' : cat;
      const isActive = (!catParam && cat === 'all') || cat === catParam;
      return `<div class="tab${isActive ? ' active' : ''}" data-cat="${escapeAttr(cat)}" onclick="filterCat(this)">${escapeHtml(label)} <span class="tab-n">${count}</span></div>`;
    }).join('');
  }

  if (catParam) currentCat = decodeURIComponent(catParam.replace(/\+/g,' '));

  // Build material filters from loaded products
  const materials = [...new Set(allProducts.map(p => p.material).filter(Boolean))];
  document.getElementById('materialFilter').innerHTML = materials.map(m => `
    <label class="fp-opt"><input type="checkbox" value="${escapeAttr(m)}" class="mat-filter" onchange="renderGrid()"> ${escapeHtml(m)}</label>`).join('');
